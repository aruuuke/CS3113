...
const char  SPRITESHEET_FILEPATH[]  = "assets/aang.png",
            PLATFORM_FILEPATH[]     = "assets/grass.png",
            ENEMY_FILEPATH[]        = "assets/zuko.png";
            
const char ENEMY2_TEXTURE_FILEPATH[] = "assets/toph.png";
const char ENEMY3_TEXTURE_FILEPATH[] = "assets/iroh.png";
const char FONT_TEXTURE[] = "assets/font1.png"; // font
...
void initialise()
{
    ...
    g_game_state.platforms[0].m_texture_id = platform_texture_id;
    g_game_state.platforms[0].set_position(glm::vec3(-5.0f, -1.0f, 0.0f));
    g_game_state.platforms[0].set_width(0.5f);
    g_game_state.platforms[0].update(0.0f, NULL, NULL, 0);

    
    // initialize the middle platforms
    for (int i = 1; i < PLATFORM_COUNT - 5; i++){
        //std::cout << i << " " << -5.0f + i<< std::endl;
        g_game_state.platforms[i].m_texture_id = platform_texture_id;
        g_game_state.platforms[i].set_position(glm::vec3(-5.0f + i, -2.0f, 0.0f));
        g_game_state.platforms[i].set_width(0.5f);
        g_game_state.platforms[i].update(0.0f, NULL, NULL, 0);
    }
 
    for (int i = 6; i < PLATFORM_COUNT-1; i++){
        //std::cout << i << " " << -5.0f + i<< std::endl;
        g_game_state.platforms[i].m_texture_id = platform_texture_id;
        g_game_state.platforms[i].set_position(glm::vec3(-5.0f + i, -1.0f, 0.0f));
        g_game_state.platforms[i].set_width(0.5f);
        g_game_state.platforms[i].update(0.0f, NULL, NULL, 0);
    }
    ...
    g_game_state.platforms[PLATFORM_COUNT - 1].set_position(glm::vec3(5.0f, 0.0f, 0.0f));
    g_game_state.platforms[PLATFORM_COUNT - 1].set_width(0.5f);
    ...
    g_game_state.player->set_position(glm::vec3(-4.9f, -1.0f, 0.0f));
    ...
    GLuint enemy2_texture_id = load_texture(ENEMY2_TEXTURE_FILEPATH);
    GLuint enemy3_texture_id = load_texture(ENEMY3_TEXTURE_FILEPATH);
    ...
    g_game_state.enemies[0].set_ai_type(GUARD); // Guard AI
    g_game_state.enemies[0].set_ai_state(IDLE);
    g_game_state.enemies[0].m_texture_id = enemy_texture_id;
    g_game_state.enemies[0].set_position(glm::vec3(0.0f, -2.0f, 0.0f)); // Position
    g_game_state.enemies[0].set_movement(glm::vec3(0.0f));
    g_game_state.enemies[0].set_speed(0.5f); // Speed
    g_game_state.enemies[0].set_acceleration(glm::vec3(0.0f, -9.81f, 0.0f));

    // Enemy 2: Walker (toph)
    g_game_state.enemies[1].set_entity_type(ENEMY);
    g_game_state.enemies[1].set_ai_type(WALKER); // Walker AI
    g_game_state.enemies[1].set_ai_state(WALKING); // Start walking immediately
    g_game_state.enemies[1].m_texture_id = enemy2_texture_id;
    g_game_state.enemies[1].set_position(glm::vec3(5.0f, 0.0f, 0.0f)); // Different position
    g_game_state.enemies[1].set_movement(glm::vec3(0.0f));
    g_game_state.enemies[1].set_speed(1.0f); // Same speed
    ...
    g_game_state.enemies[2].set_speed(0.3f);
...
void update()
{
    float ticks = (float)SDL_GetTicks() / MILLISECONDS_IN_SECOND;
        ...
        while (delta_time >= FIXED_TIMESTEP) {
            ...
            g_game_state.player->check_collision_enemies(g_game_state.enemies,ENEMY_COUNT); // hhhh
...
void render()
{
    ...
    if (g_game_state.player->health >= 0 && g_game_state.player->dead == ENEMY_COUNT) {
           glClear(GL_COLOR_BUFFER_BIT);
           draw_text(&g_shader_program, load_texture(FONT_TEXTURE), "YOU WIN!", 0.5f, -0.15f, glm::vec3(-1.0f, 0.0f, 0.0f));
            g_game_state.player->deactivate();
    }
    
    if (g_game_state.player->health < 0) {
        glClear(GL_COLOR_BUFFER_BIT);
        draw_text(&g_shader_program, load_texture(FONT_TEXTURE), "YOU LOSE!", 0.5f, -0.15f, glm::vec3(-1.0f, 0.0f, 0.0f));
         g_game_state.player->deactivate();
    }
